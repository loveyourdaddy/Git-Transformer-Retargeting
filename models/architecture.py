...
class GAN_model(BaseModel):
    def __init__(self, args, character_names, dataset):
        ...
        self.args = args
        if self.is_train:
            self.split_index = dataset.split_index # check when test 
        ...
        if self.is_train:
            ...
            self.criterion_rec = torch.nn.MSELoss() #1
            self.criterion_gan = GAN_loss(args.gan_mode).to(self.device) #3
            self.criterion_cycle = torch.nn.L1Loss() #2
            self.criterion_ee = Criterion_EE(args, torch.nn.MSELoss()) #4
            for i in range(self.n_topology):
            ...
            if self.is_valid is False:
                ...
                self.bvh_path = os.path.join(args.save_dir, 'results_validation/bvh')
    ...
    def backward_G(self):
        ...
        for i in range(self.n_topology):
            ...
            if self.is_train==True and i==1: 
                root_pos_weight, joint_weight = 100, 100
                rec_loss1 = self.weighted_criterion_rec(self.motions[i], self.res[i], self.split_index, root_pos_weight, joint_weight) 
            else:
                rec_loss1 = self.criterion_rec(self.motions[i], self.res[i])
    ...
    # rec_loss1 = self.criterion_rec_weight(self.motions[i], self.res[i], new_joint_index, weight) 
    def weighted_criterion_rec(self, motions, res, joint_index, root_pos_weight, joint_weight):
        
        # calculate all loss vector 
        loss_vec = (motions - res )**2
        
        for d in range(0,3):
            loss_vec[:, d, :] *= root_pos_weight

        # multiply the factors 
        for i in joint_index:
            for d in range(0,4): 
                loss_vec[:, 3 + i*4 + d, :] *= joint_weight
    ...
    # ----    
    def save_two_model(self, topology0, epoch0, topology1, epoch1):
        model = self.models[0]
        file_name = os.path.join(self.model_save_dir, 'topology{}'.format(topology0))
        model.save(file_name, epoch0)

        model = self.models[1]
        file_name = os.path.join(self.model_save_dir, 'topology{}'.format(topology1))
        model.save(file_name, epoch1)


        optimizer = self.optimizers[0]
        file_name = os.path.join(self.model_save_dir, 'optimizers/{}/{}.pt'.format(epoch0, topology0))
        try_mkdir(os.path.split(file_name)[0])
        torch.save(optimizer.state_dict(), file_name)
        
        optimizer = self.optimizers[1]
        file_name = os.path.join(self.model_save_dir, 'optimizers/{}/{}.pt'.format(epoch1, topology1))
        try_mkdir(os.path.split(file_name)[0])
        torch.save(optimizer.state_dict(), file_name)

    def load_one_model(self, topology, epoch):
        model = self.models[0]
        model.load(os.path.join(self.model_save_dir, 'topology{}'.format(topology)), epoch) 

        # if self.is_train:
        #     optimizer = self.optimizers[0]
        #     file_name = os.path.join(self.model_save_dir, 'optimizers/{}/{}.pt'.format(epoch, topology))
        #     print(file_name)
        #     optimizer.load_state_dict(torch.load(file_name))

    def load_two_model(self, topology0, epoch0, topology1, epoch1):
        model = self.models[0]
        model.load(os.path.join(self.model_save_dir, 'topology{}'.format(topology0)), epoch0)  
        model = self.models[1]
        model.load(os.path.join(self.model_save_dir, 'topology{}'.format(topology1)), epoch1) 
    ...
    def compute_test_result(self):
        ...
        for src in range(self.n_topology):
            ...
            for i in idx:
                ...
                #print(os.path.join(new_path, '{}_gt.bvh'.format(self.id_test)))
        ...
        for src in range(self.n_topology):
            for dst in range(self.n_topology):
                for i in range(len(self.character_names[dst])):
                    ...
                    # print(os.path.join(dst_path, '{}_{}.bvh'.format(self.id_test, src)))
                p += 1
