...
class SkeletonPool(nn.Module):
    def __init__(self, edges, pooling_mode, channels_per_edge, last_pool=False):
        ...
        def find_seq(j_indx, seq):
            nonlocal self, degree, edges

            #조건1: 0 일때 제외하고 degree > 2 일때 
            if degree[j_indx] > 2 and j_indx != 0:
                self.seq_list.append(seq)
                seq = []
            
            #조건1: end effector (degree = 1) 일때 
            if degree[j_indx] == 1:
                self.seq_list.append(seq)
                return 

            for idx, edge in enumerate(edges): # enumerate는 이미 거친건 확인하지 않는가?
                if edge[0] == j_indx: # 모든 edge 중에(edge) j_indx와 같은 것을 찾음. 그것의 child로 다음 차례로 전달해줌. 
                    find_seq(edge[1], seq + [idx])

        find_seq(0, [])
        # print("seq_list:{}".format(self.seq_list))

        for seq in self.seq_list:
            if last_pool:
                self.pooling_list.append(seq)
                # print("last_pool:{}".format(self.pooling_list))
                continue
            if len(seq) % 2 == 1:
                self.pooling_list.append([seq[0]])
                self.new_edges.append(edges[seq[0]])
                seq = seq[1:]
                # print("half case:{}\n{}\n{}".format(self.pooling_list, self.new_edges, seq))
            for i in range(0, len(seq), 2):
                self.pooling_list.append([seq[i], seq[i + 1]])
                self.new_edges.append([edges[seq[i]][0], edges[seq[i + 1]][1]])
        # print("pooling_list:{} \n new_edges:{}".format(self.pooling_list, self.new_edges))
        # pdb.set_trace()
