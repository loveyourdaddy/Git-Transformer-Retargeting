...
class MultiHeadAttention(nn.Module):
    def __init__(self, args):
        ...
        # self.W_Q = nn.Linear(self.window_size, self.n_head * self.d_head) # W: (DoF, 256)
        # self.W_K = nn.Linear(self.window_size, self.n_head * self.d_head)
        # self.W_V = nn.Linear(self.window_size, self.n_head * self.d_head)
        self.W_Q = nn.Conv1d(self.window_size, self.n_head * self.d_head, kernel_size=3, padding=1) # W: (91, 256)
        self.W_K = nn.Conv1d(self.window_size, self.n_head * self.d_head, kernel_size=3, padding=1)
        self.W_V = nn.Conv1d(self.window_size, self.n_head * self.d_head, kernel_size=3, padding=1)
    ...
    def forward(self, Q, K, V, attn_mask):
        ...
        K = K.transpose(1,2)
        V = V.transpose(1,2)
        q_s = self.W_Q(Q).transpose_(1,2).view(batch_size, -1, self.n_head, self.d_head).transpose(1, 2)
        k_s = self.W_K(K).transpose_(1,2).view(batch_size, -1, self.n_head, self.d_head).transpose(1, 2)
        v_s = self.W_V(V).transpose_(1,2).view(batch_size, -1, self.n_head, self.d_head).transpose(1, 2)
...
class PositionFeedForwardNet(nn.Module):
    def __init__(self, args):
        ...
        # Layer
        self.conv1 = nn.Conv1d(in_channels = self.window_size, out_channels = self.window_size * 4, kernel_size=3, padding=1)
        self.conv2 = nn.Conv1d(in_channels = self.window_size * 4, out_channels = self.window_size, kernel_size=3, padding=1)
        self.active = F.gelu
...
def get_attn_pad_mask(seq_q, seq_k, i_pad):
    ...
    # seq k에서 0인 부분을 <pad
    # for Conv2d
    # seq_q = torch.squeeze(seq_q, 0)
    # seq_k = torch.squeeze(seq_k, 0)
    valueTensor = torch.bmm(seq_q, seq_k.transpose(1,2)) # batch matrix mult

    # for Conv2d
    # valueTensor = torch.unsqueeze(valueTensor, 0)
...
class Encoder(nn.Module):
    def __init__(self, args):
        ...
        self.conv1 = nn.Conv1d(self.window_size, self.window_size, kernel_size=3, padding=1)
    ...
    def forward(self, inputs):
        ...
        outputs = inputs.transpose(1,2) # (4,64,91)

        # (window)
        frames_tensor = torch.arange(outputs.size(1), device=outputs.device, dtype=torch.int)
        # (window) -> (1,window,1)
        tmp1 = torch.unsqueeze(torch.unsqueeze(frames_tensor, 0), 2)
        # (1,window,1) -> (1,window,DoF)
        one_motion_tensor = tmp1.expand(-1, -1, outputs.size(2))
        # (1,window,DoF) -> (bs,window,DoF)
        positions = one_motion_tensor.expand(outputs.size(0), -1, -1)
        
        # (bs, window, Dof)
        pos_mask = outputs.eq(self.args.i_pad)
...
class Decoder(nn.Module):
    def __init__(self, args):
        ...
        # output(depth)의 갯수이니, 2차원으로 써줘야함 
        # self.fc1 = nn.Linear(self.window_size, self.window_size)
        self.conv1 = nn.Conv1d(self.window_size, self.window_size, kernel_size=1, padding=0) # 차원 뒤집은 다음에사용하기 
        # self.conv1 = nn.Conv2d(1, 1, kernel_size=3, padding=1)
    ...
    def forward(self, dec_inputs, enc_inputs, enc_outputs):
        ...
        # conv 연산을 위한 뒤집기 
        dec_outputs = dec_inputs.transpose(1,2)
        
        # (64) -> (1,1,64)
        frames_tensor = torch.arange(dec_outputs.size(1), device=dec_outputs.device, dtype=torch.int)
        tmp1 = torch.unsqueeze(torch.unsqueeze(frames_tensor, 0), 2)
        # (1,1,64) -> (1,91,64)
        one_motion_tensor = tmp1.expand(-1, -1, dec_outputs.size(2))
        # (1,91,64) -> (4,91,64)
        positions = one_motion_tensor.expand(dec_outputs.size(0), -1, -1)
        
        pos_mask = dec_outputs.eq(self.args.i_pad)
        positions = positions.contiguous()
        ...
        for layer in self.layers:
            ...
            # print("layer: {}, dec_outputs: {}".format(i, dec_outputs[0][0]))
...
class MotionGenerator(nn.Module):
    def __init__(self, args, character_names, dataset):
        ...
        self.projection = nn.Conv1d(self.window_size, self.window_size, kernel_size=3, padding=1)
